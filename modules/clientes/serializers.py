...
class ClienteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            ...
            "total_notas", "total_eventos",
            "created_at", "updated_at", "created_by",
        ]

    def get_total_notas(self, obj):
        # Uses prefetch_related("notas_detalle") from the ViewSet queryset
        if hasattr(obj, "_prefetched_objects_cache") and "notas_detalle" in obj._prefetched_objects_cache:
            return len(obj._prefetched_objects_cache["notas_detalle"])
        return obj.notas_detalle.filter(deleted_at__isnull=True).count()

    def get_total_eventos(self, obj):
        if hasattr(obj, "_prefetched_objects_cache") and "historial" in obj._prefetched_objects_cache:
            return len(obj._prefetched_objects_cache["historial"])
