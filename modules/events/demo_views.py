...
class DemoResourcesView(APIView):
    ...
    def get(self, request):
        try:
            ...
            logger.error(f"DemoResourcesView Error: {str(e)}", exc_info=True)
            # Return detailed error so we can see it in the UI alert
            return Response({"error": f"Internal error during sync: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
